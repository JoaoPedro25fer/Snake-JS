...
function atualizar_jogo() {
    ...
    // passagens laterais
    if (snake.corpo[0].x > 1115) {
        snake.corpo[0].x = -15;
    } else if (snake.corpo[0].x < -15) {
        snake.corpo[0].x = 1115;
    }
...
function desenhar_jogo() {
    ...
    if (alim.ativo) {
        ...
        ctx.fillStyle = '#0009';
        ctx.beginPath();
        ctx.arc(alim.x + 4, alim.y + 4, alim.raio, 0, 2 * Math.PI);
        ctx.fill();
        // alimento
        ctx.fillStyle = '#f00';
        ctx.beginPath();
        ctx.arc(alim.x, alim.y, alim.raio, 0, 2 * Math.PI);
        ctx.fill();
    }
